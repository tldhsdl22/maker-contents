...
포스팅된 콘텐츠의 성과를 자동으로 추적합니다. 네이버 검색에서의 키워드 순위를 주기적으로 수집하여 포스팅의 효과를 측정합니다.
...
포스팅 완료 후 7일간 3시간 간격으로 다음 데이터를 수집합니다.
...
2. 3시간 간격으로 다음을 수행합니다:
   - 네이버 검색 API에서 등록된 키워드로 검색하여 해당 포스팅의 순위를 확인합니다.
...
상단에 현재 순위를 요약 카드로 표시합니다. 하단에 시간별 추이 그래프(키워드 순위)를 표시합니다. 추적 기간(7일)과 현재 진행 상황을 표시합니다.
