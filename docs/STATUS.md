...
### 2026-02-23 - 원고 제목 입력 너비 개선
- **프론트엔드 (client/)**
  - 원고 수정 페이지 제목 입력창을 가변 너비로 변경
  - 헤더 좌측 영역을 flex 확장해 제목 입력 공간 확보

### 2026-02-23 - 포스팅 완료 모달 위치 고정
- **프론트엔드 (client/)**
  - 포스팅 완료 모달을 portal로 렌더링해 스크롤 위치와 무관하게 표시

### 2026-02-20 (37차) - 성과 추적 순위 API 전환
- **백엔드 (server/)**
  - 네이버 검색 API 기반으로 블로그/카페 순위 수집 로직 적용
  - 검색 API 키 풀(복수 키) 순차 로테이션 지원
  - 성과 수집 스케줄러 3시간 간격으로 조정
  - 조회수/댓글 수집은 비활성화 (순위 중심)
- **프론트엔드 (client/)**
  - 성과/대시보드/원고 목록에서 조회/댓글 UI 제거, 순위 중심 표시

### 2026-02-19 (36차) - 원고 생성 설정 기억
- **프론트엔드 (client/)**
  - 원고 생성 페이지에서 마지막 설정(프롬프트/템플릿/키워드/길이/이미지 개수) 저장 후 자동 복원

### 2026-02-19 (35차) - 카페 성과 실측 수집
- **백엔드 (server/)**
  - 카페 포스팅 URL에서 cafeId/articleId 추출 후 조회수/댓글 수 실측 수집 적용
  - 순위는 기존 mock 유지 (조회/댓글만 실측)

### 2026-02-19 (29차) - 전역 UI 톤 개선
- **프론트엔드 (client/)**
  - 글로벌 컬러/그라데이션/버튼 톤을 더 생동감 있게 조정

### 2026-02-19 (30차) - 포스팅 완료 모달 URL 보강
- **프론트엔드 (client/)**
  - URL 프로토콜 자동 보정 및 카페 모바일 도메인 감지 추가

### 2026-02-19 (31차) - 원고 생성 제목 자동화
- **백엔드 (server/)**
  - 생성된 원고 본문을 기반으로 제목을 자동 생성하여 저장

### 2026-02-19 (32차) - 대시보드 레이아웃 정리
- **프론트엔드 (client/)**
  - 요약/성과/빠른 접근 섹션 구분 및 카드 톤 정리

### 2026-02-19 (33차) - 주요 페이지 섹션 정돈
- **프론트엔드 (client/)**
  - 소스/원고/성과 페이지 카드 분리 및 헤더 구조 정리

### 2026-02-19 (34차) - 버튼 인터랙션 명확화
- **프론트엔드 (client/)**
  - 버튼/버튼 역할 요소에 포인터 커서 및 비활성 커서 적용

### 2026-02-19 (35차) - 원고 복사 UX 개선
- **프론트엔드 (client/)**
  - 복사 버튼 fallback 시 HTML 렌더링 내용을 클립보드에 복사

### 2026-02-19 (24차) - 배포용 gitignore 정리
- **공통**
  - 업로드/로그/런타임 파일 제외 규칙 보강 (root/server/client)

### 2026-02-19 (28차) - 이미지 S3 강제 저장
- **백엔드 (server/)**
  - 원고 이미지 저장 시 로컬 fallback 제거, S3 업로드만 사용하도록 변경

### 2026-02-19 (25차) - 서버 빌드 타입 오류 수정
- **백엔드 (server/)**
  - JWT 옵션 타입/LLM 응답 타입 보강
  - Cheerio 옵션 타입 충돌 제거

### 2026-02-19 (26차) - 프론트 빌드 타입 오류 수정
- **프론트엔드 (client/)**
  - TipTap StarterKit 옵션 타입/auto save ref 초기값 수정
...
### 2026-02-20: 성과 추적 지표 단순화
- 네이버 검색 API 순위 추적만 유지하고 조회수/댓글 수집은 제거
- 성과 수집 주기를 3시간 간격으로 조정
